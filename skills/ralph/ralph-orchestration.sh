...
find_ralph() {
    ...
    if [ -n "${RALPH_PATH:-}" ] && [ -f "$RALPH_PATH" ]; then
        log INFO "使用环境变量 RALPH_PATH: $RALPH_PATH"
        return 0
    fi
    
    # 2. 从脚本位置推断（skills/ralph/ 的上级目录的上级目录）
    local script_dir
    script_dir="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
    local inferred_path="$script_dir/../../ralph.sh"
    
    if [ -f "$inferred_path" ]; then
        RALPH_PATH="$inferred_path"
        log INFO "从脚本位置推断：$RALPH_PATH"
        return 0
    fi
    ...
    if command -v ralph.sh &> /dev/null; then
        ...
        log INFO "从 PATH 中找到：$RALPH_PATH"
        return 0
    fi
    
    # 4. 常见位置搜索（不包含硬编码的开发路径）
    local search_paths=(
        "$PWD/ralph.sh"
        "$(pwd)/ralph.sh"
        "$HOME/.local/bin/ralph.sh"
        "/usr/local/bin/ralph.sh"
        "$HOME/workspace/*/ralph.sh"
    )
    ...
    for path in "${search_paths[@]}"; do
        ...
        for expanded_path in $path; do
            if [ -f "$expanded_path" ]; then
                RALPH_PATH="$expanded_path"
                log INFO "从常见位置找到：$RALPH_PATH"
                return 0
            fi
        done
    done
    
    log ERROR "未找到 ralph.sh，请设置环境变量 RALPH_PATH"
    return 1
}
